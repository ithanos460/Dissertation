# -*- coding: utf-8 -*-
"""
Created on Wed Oct  5 15:10:27 2022
...
dim_geogr2 = uscities.drop_duplicates(['state_id','county_fips','id'])[['state_id','county_fips','id']]
dim_geogr2['dim_geo_id'] = dim_geogr2.id.map(hash)

# dim state
dim_geogr_states = uscities.drop_duplicates(['state_name','state_id'])[['state_name','state_id']]

# dim county
dim_geogr_counties = uscities.drop_duplicates(['county_name','county_fips'])[['county_name','county_fips']]

# dim city
dim_geogr_cities = uscities.drop_duplicates(['city','id'])[['city','id']]
...
# dim year
dim_year = calendar.drop_duplicates(['year'])[['year']]
dim_year['dim_year_id'] = dim_year.year.map(hash)

# dim quarter
dim_quarter= calendar.drop_duplicates(['quarter'])[['quarter']]
dim_quarter['dim_quarter_id'] = dim_quarter.quarter.map(hash)

# dim week
dim_week= calendar.drop_duplicates(['week'])[['week']]
dim_week['dim_week_id'] = dim_week.week.map(hash)

dim_calendar = dim_calendar.rename(
    columns={'year': 'dim_year_id', 'quarter': 'dim_quarter_id', 'week': 'dim_week_id'})
...
covid['year'] = pd.DatetimeIndex(covid['week_ending']).year
covid['month'] = pd.DatetimeIndex(covid['week_ending']).month
covid['day'] = pd.DatetimeIndex(covid['week_ending']).day
...
cancer_unpivot = cancer_unpivot.rename(columns={'Cancer Type': 'Disease'})
cancer_unpivot = cancer_unpivot[['Disease', 'FIPS', 'Year', 'KPI', 'Value']]
...
#######################################################################################
dim_diseas.to_sql("snow_dim_diseas", engine, schema='Curated')
dim_geogr2.to_sql("snow_dim_geogr", engine, schema='Curated')
dim_geogr_states.to_sql("snow_dim_states", engine, schema='Curated')
dim_geogr_counties.to_sql("snow_dim_counties", engine, schema='Curated')
dim_geogr_cities.to_sql("snow_dim_cities", engine, schema='Curated')
dim_calendar.to_sql("snow_dim_calendar", engine, schema='Curated')
dim_year.to_sql("snow_dim_year", engine, schema='Curated')
dim_quarter.to_sql("snow_dim_quarter", engine, schema='Curated')
dim_week.to_sql("snow_dim_week", engine, schema='Curated')
dim_holidays.to_sql("snow_dim_holidays", engine, schema='Curated')
poverty_unpivot.to_sql("snow_fact_poverty", engine, schema='Curated')
covid.to_sql("snow_fact_covid", engine, schema='Curated')
cancer_unpivot.to_sql("snow_fact_cancer", engine, schema='Curated')
hiv_unpivot.to_sql("snow_fact_hiv", engine, schema='Curated')
us_pop_unpivot.to_sql("snow_fact_demogr", engine, schema='Curated')
unemployment_unpivot.to_sql("snow_fact_unempl", engine, schema='Curated')
fact_temper_f.to_sql("snow_fact_temps", engine, schema='Curated')
education_unpivot.to_sql("snow_fact_educ", engine, schema='Curated')
